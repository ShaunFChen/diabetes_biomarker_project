...
@patch("builtins.open", new_callable=mock_open, read_data="subject_id,mtb_0018261,BMI,age,sex\n")
...
def test_load_data(mock_read_csv, mock_check_file_exists, mock_file):
    ...
    # Act
    df = load_data(raw_data_path, categorical_columns, numeric_columns, biomarker_pattern)

    # Assert
    mock_check_file_exists.assert_called_once_with(Path(raw_data_path))
    mock_file.assert_called_once_with(raw_data_path, "r")
    mock_read_csv.assert_called_once_with(raw_data_path, dtype={
        "subject_id": "object", "sex": "object", "BMI": np.float64, "age": np.float64, "mtb_0018261": np.float64
    }, engine="pyarrow")
