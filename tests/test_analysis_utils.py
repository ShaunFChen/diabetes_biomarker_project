...
def test_match_with_replacement():
    ...
    data = {
        ...
        "other_feature": [
            ...
            30,
            40,
        ],  # Additional column to check data integrity
    }
    df = pd.DataFrame(data)
    outcome_col = "outcome"
...
def test_match_without_replacement():
    # Create sample data with sufficient controls
    data = {"propensity_score": [0.1, 0.3, 0.5, 0.7], "outcome": [0, 0, 1, 1]}
    df = pd.DataFrame(data)
    outcome_col = "outcome"
...
def test_match_without_replacement_insufficient_controls():
    ...
    data = {"propensity_score": [0.2, 0.4, 0.6], "outcome": [0, 1, 1]}
    df = pd.DataFrame(data)
    outcome_col = "outcome"
    ...
    pd.testing.assert_frame_equal(
        matched_df.reset_index(drop=True), df.reset_index(drop=True)
    )
...
def test_run_single_logistic_regression():
    ...
    # Run the function
    log2_fc, p_value = run_single_logistic_regression(
...
def test_run_logistic_regression_mp():
    ...
    # Run the function
    log2_fcs, p_values = run_logistic_regression_mp(
        df, covariates, outcome_col, biomarker_cols, n_cores=2
    )
...
def test_filter_highly_correlated_features():
    ...
    features_to_drop = filter_highly_correlated_features(
        df, significant_biomarkers, threshold=0.9
    )
